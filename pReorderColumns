...
DECLARE @currentIdentity SQL_VARIANT = (SELECT last_value FROM sys.identity_columns WHERE object_id = @tableObjectId)
...
DECLARE @statements TABLE(ID INT NOT NULL PRIMARY KEY IDENTITY(1, 1), Error NVARCHAR(MAX), DropStatement NVARCHAR(MAX), CreateStatement NVARCHAR(MAX))
...
--Statements für default-constraints
INSERT @statements(Error, DropStatement, CreateStatement)
	SELECT 
...
INSERT @statements (Error, DropStatement, CreateStatement)
	SELECT 
		CASE WHEN indexes.data_space_id != 1 or indexes.ignore_dup_key != 0 OR indexes.is_unique_constraint != 0 OR indexes.fill_factor != 0 OR indexes.is_padded != 0 OR indexes.is_disabled != 0 OR indexes.is_hypothetical != 0 OR indexes.allow_row_locks != 1 OR indexes.allow_page_locks != 1 THEN 'Unsupported index found' ELSE NULL END,
		CASE WHEN indexes.is_primary_key = 1 THEN 'ALTER TABLE [' + @schemaName + '].[' + tables.name + '] DROP CONSTRAINT [' + indexes.name + ']' ELSE 'DROP INDEX [' + indexes.name + '] ON [' + @schemaName + '].[' + tables.Name + ']' END,
...
--Statements for outgoint foreign keys
INSERT @statements (Error, DropStatement, CreateStatement)
	SELECT 
...
INSERT @statements (Error, DropStatement, CreateStatement)
	SELECT 
		CASE WHEN indexes.data_space_id != 1 or indexes.ignore_dup_key != 0 OR indexes.is_unique_constraint != 0 OR indexes.fill_factor != 0 OR indexes.is_padded != 0 OR indexes.is_disabled != 0 OR indexes.is_hypothetical != 0 OR indexes.allow_row_locks != 1 OR indexes.allow_page_locks != 1 THEN 'Unsupported index found' ELSE NULL END,
		CASE WHEN indexes.is_primary_key = 1 THEN CONVERT(NVARCHAR(MAX), CONVERT(INT, '<Error> There cannot be a primary key on a view.')) ELSE 'DROP INDEX [' + indexes.name + '] ON [' + schemas.name + '].[' + views.Name + ']' END,
		CASE WHEN indexes.is_primary_key = 1 THEN
					...
					'[' + columns.name + '] ' +
						CASE 
							...
							ELSE
								...
								CASE WHEN columns.name = @identityColumn THEN ' IDENTITY(1, 1)' ELSE '' END
						END +
						',' 
				END AS [text()]
				FROM sys.columns
...
END ELSE BEGIN
	...
	DECLARE execCursor CURSOR FOR SELECT DropStatement FROM @statements ORDER BY ID DESC
	OPEN execCursor
	FETCH NEXT FROM execCursor INTO @toExecute
	WHILE @@FETCH_STATUS = 0 BEGIN
		PRINT 'Executing: ' + @toExecute
		EXEC sp_executesql @stmt = @toExecute
		FETCH NEXT FROM execCursor INTO @toExecute
	END
	CLOSE execCursor
	...
	DECLARE @tableFullName NVARCHAR(MAX) = '[' + @schemaName + '].[' + @table + ']'
	PRINT 'Renaming the table now.'
	EXEC sp_rename @objname = @tableFullName, @newname = @tempName
	DECLARE @stmt NVARCHAR(MAX) = 'CREATE TABLE [' + @schemaName + '].[' + @table + '] (' + LEFT(@columnDefinitions, LEN(@columnDefinitions) - 1) + ')'
	...
	IF @identityColumn IS NOT NULL BEGIN
		SET @stmt = @stmt + 'SET IDENTITY_INSERT ' + @table + ' ON; '
	END
	SET @stmt = @stmt + 'INSERT ' + @table + '(' + LEFT(@selectColumnsOldTable, LEN(@selectColumnsOldTable)-1) + ') SELECT ' + LEFT(@selectColumnsOldTable, LEN(@selectColumnsOldTable)-1) + ' FROM ' + @tempName + ';'
	IF @identityColumn IS NOT NULL BEGIN
		SET @stmt = @stmt + ' SET IDENTITY_INSERT ' + @table + ' OFF'
	END
	PRINT 'Executing: ' + @stmt
	EXEC sp_executesql @stmt = @stmt

	SET @stmt = 'DROP TABLE ' + @tempName
	PRINT 'Executing: ' + @stmt
	EXEC sp_executesql @stmt = @stmt

	IF @identityColumn IS NOT NULL BEGIN
		SET @stmt = 'DBCC CHECKIDENT (' + @table + ', RESEED, ' + CONVERT(NVARCHAR(100), @currentIdentity) + ')'
		PRINT 'Executing: ' + @stmt
		EXEC sp_executesql @stmt = @stmt
	END

	--Create everything that was dropped before.
	DECLARE execCursor CURSOR FOR SELECT CreateStatement FROM @statements ORDER BY ID ASC
	OPEN execCursor
	FETCH NEXT FROM execCursor INTO @toExecute
	WHILE @@FETCH_STATUS = 0 BEGIN
		PRINT 'Executing: ' + @toExecute
